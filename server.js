...
async function pollGenerationStatus(taskId) {
  ...
  const data = await res.json();
  // TEMP DEBUG: log the raw shape so we can confirm the real field names —
  // remove once confirmed working.
  console.log(`[aetherwave debug] status poll for ${taskId}:`, JSON.stringify(data));
  return data;
}
...
app.get('/api/sovereign/produce/:taskId', async (req, res) => {
  ...
  try {
    ...
    // CONFIRMED via live logs (2026-08-31): AetherWave's real status values are
    // uppercase — PENDING (generating), FIRST_SUCCESS (one track ready),
    // SUCCESS (both tracks ready). The original 'complete'/'processing'/'failed'
    // guesses never matched, which is why generations silently ran to
    // completion on AetherWave's side while this endpoint kept reporting
    // "processing" until the frontend timed out.
    const status = String(remote.status || '').toUpperCase();
    // Failure values are unconfirmed (no failure has been observed yet) — these
    // are the standard terminal-failure names used by this class of music-gen
    // API (the audioUrl domains, aiquickdraw.com / kie.ai, match that family).
    // Treat any of them, or an explicit `failed`, as failed; verify/adjust if
    // a real failure ever surfaces a different value.
    const FAILURE_STATUSES = ['FAILED', 'CREATE_TASK_FAILED', 'GENERATE_AUDIO_FAILED', 'CALLBACK_EXCEPTION', 'SENSITIVE_WORD_ERROR'];

    if (status === 'SUCCESS' && Array.isArray(remote.tracks) && remote.tracks.length > 0) {
      await ingestGeneratedTracks(task, remote.tracks);
    } else if (FAILURE_STATUSES.includes(status) || remote.status === 'expired_or_not_found') {
      task.status = 'failed';
      task.error = remote.error ?? (remote.status === 'expired_or_not_found' ? 'Task not found — AetherWave job IDs expire after 7 days' : `AetherWave reported generation failure (status: ${remote.status})`);
    } else {
      // PENDING, FIRST_SUCCESS, or any other in-progress value — keep polling.
