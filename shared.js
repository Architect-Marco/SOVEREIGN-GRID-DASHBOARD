        ...
        window.SOVEREIGN_12_PLUGINS = [
            ...
            { id: 'vocal-rider', name: 'Voc Ride', tagline: 'Formant Harmonic EQ', category: 'EQ',
              values: [['1ST','-6dB'],['2ND','+4dB'],['3D','-3dB'],['NTH POS','3.0kHz']],
              presets: [
                { name: 'sovereign: default preset', values: [['1ST','-6dB'],['2ND','+4dB'],['3D','-3dB'],['NTH POS','3.0kHz']] },
                { name: 'sovereign: bright lead vocal', values: [['1ST','-3dB'],['2ND','+6dB'],['3D','+2dB'],['NTH POS','4.5kHz']] },
                { name: 'sovereign: warm podcast voice', values: [['1ST','+2dB'],['2ND','-2dB'],['3D','-6dB'],['NTH POS','1.8kHz']] }
              ] },
            ...
            { id: 'stereo-imager', name: 'Stereo Imager', tagline: 'Width Expansion', category: 'SPACE',
              values: [['GAIN','0.0d'],['WIDTH','2.00'],['ASYM','0'],['ROT','0.0']],
              presets: [
                { name: 'sovereign: wide master', values: [['GAIN','0.0d'],['WIDTH','2.60'],['ASYM','0'],['ROT','0.0']] },
                { name: 'sovereign: mono-safe', values: [['GAIN','0.0d'],['WIDTH','1.10'],['ASYM','0'],['ROT','0.0']] }
              ] },
        ...
        window.renderDawFxPicker = function() {
            ...
            if (ctx.mode === 'browse') {
                ...
                } else if (plugin && plugin.id === 'stereo-imager') {
        ...
        // ============================================================
        // STEREO IMAGER — a Waves S1-style panel: Gain & Width vertical
        // faders, a live polar goniometer (correlation wedge, animated
        // with window.requestAnimationFrame), Asymmetry & Rotation
        // sliders, Input/Meter Mode (L-R / M-S) toggles, phase controls,
        // and L/R output LED meters.
        // ============================================================
        window.dawImagerState = window.dawImagerState || {}; // key -> {gain,width,asymmetry,rotation,inputMode,meterMode,invL,invR,swap}

        function dawImagerDefaultState() {
            return { gain: 0.0, width: 2.00, asymmetry: 0, rotation: 0.0, inputMode: 'LR', meterMode: 'LR', invL: false, invR: false, swap: false };
        }
        function dawImagerGetState(key) {
            if (!window.dawImagerState[key]) window.dawImagerState[key] = dawImagerDefaultState();
            return window.dawImagerState[key];
        }
        function dawImgFmtGain(v) { return (v >= 0 ? '+' : '') + v.toFixed(1); }

        const DAW_IMG_GRAPH_W = 280, DAW_IMG_GRAPH_H = 190;
        const DAW_IMG_RINGS = [ [118,'0'], [88.5,'-6'], [59,'-12'], [29.5,'-18'] ]; // radius -> label, innermost (-24) is the apex itself

        function dawImagerGeometry(s, jitter) {
            const apexX = DAW_IMG_GRAPH_W / 2 + (s.asymmetry / 90) * 36;
            const apexY = DAW_IMG_GRAPH_H - 14;
            const spread = (s.width / 3) * 118;
            const j = jitter || 0;
            return {
                apexX, apexY,
                leftX: apexX - spread + j * 0.6,
                rightX: apexX + spread - j * 0.4,
                topY: 14
            };
        }

        function dawImagerGoniometerInner(key, s, jitter) {
            const g = dawImagerGeometry(s, jitter);
            const rings = DAW_IMG_RINGS.map(([r, label]) => `
                <path d="M ${(g.apexX - r).toFixed(1)} ${g.apexY} A ${r} ${r} 0 0 1 ${(g.apexX + r).toFixed(1)} ${g.apexY}" fill="none" stroke="rgba(47,208,255,0.22)" stroke-width="1"/>
                <text x="${(g.apexX - r).toFixed(1)}" y="${g.apexY + 11}" font-size="8" fill="#2fd0ff" text-anchor="middle" opacity="0.75">${label}</text>
                <text x="${(g.apexX + r).toFixed(1)}" y="${g.apexY + 11}" font-size="8" fill="#2fd0ff" text-anchor="middle" opacity="0.75">${label}</text>`).join('');
            return `
                <rect x="0" y="0" width="${DAW_IMG_GRAPH_W}" height="${DAW_IMG_GRAPH_H}" fill="#000"/>
                ${rings}
                <text x="${g.apexX.toFixed(1)}" y="${g.apexY + 11}" font-size="8" fill="#2fd0ff" text-anchor="middle">-24</text>
                <g transform="rotate(${s.rotation} ${g.apexX.toFixed(1)} ${g.apexY.toFixed(1)})">
                    <line x1="${g.apexX.toFixed(1)}" y1="${g.apexY}" x2="${g.apexX.toFixed(1)}" y2="${g.topY}" stroke="rgba(47,208,255,0.35)" stroke-width="1"/>
                    <polygon points="${g.leftX.toFixed(1)},${g.topY} ${g.rightX.toFixed(1)},${g.topY} ${g.apexX.toFixed(1)},${g.apexY}" fill="rgba(47,208,255,0.28)" stroke="#2fd0ff" stroke-width="1.6"/>
                </g>`;
        }

        window.dawImagerPanel = function(key, plugin) {
            const s = dawImagerGetState(key);
            const sk = eq8SafeKey(key);
            try { window.dawStartMeterLoop(); } catch (e) {}
            try { window.dawImagerAnimate(key); } catch (e) {}
            return `
            <div class="flex items-start justify-between gap-2 mb-1">
                <div class="min-w-0">
                    <div class="neon-blue-text text-sm font-black italic truncate">${plugin.name}</div>
                ...
                <span class="relative inline-block flex-shrink-0">
                    <button onclick="event.stopPropagation(); window.dawImagerTogglePresetMenu('${key}')" class="flex items-center gap-1 text-[8px] font-black uppercase tracking-widest neon-blue-text border border-[rgba(47,208,255,0.5)] rounded px-2 py-1 bg-black/50 hover:bg-[#2fd0ff]/15 transition-colors max-w-[150px]">
                        <span id="dawimg-badge-${sk}" class="truncate">${window.dawFxActivePresetLabel[key] || 'factory preset'}</span>
                        <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" class="flex-shrink-0"><path d="M6 9l6 6 6-6"/></svg>
                    </button>
                    <div id="dawimg-preset-menu-${sk}" class="hidden-section"></div>
                </span>
            </div>
            <div class="inline-block text-[8px] neon-blue-text uppercase font-black tracking-widest mt-1 border border-[rgba(47,208,255,0.35)] rounded px-1.5 py-0.5">${plugin.category}</div>

            <div class="flex gap-3 mt-4">
                <!-- LEFT: Gain / Width faders + Input Mode -->
                <div class="flex flex-col gap-3 flex-shrink-0">
                    <div class="flex gap-2">
                        <div class="flex flex-col items-center gap-1.5">
                            ...
                            <div class="flex items-end gap-1">
                                <div class="daw-fader-scale">${dawFaderScaleHtml()}</div>
                                <div class="daw-fader-track" style="height:130px;">
                                    <input id="dawimg-gain-slider-${sk}" type="range" min="-24" max="0" step="0.1" value="${s.gain}" class="daw-fader-input eq8-fader-thumb"
                                        oninput="window.dawImagerSetField('${key}','gain', this.value)">
                                </div>
                            </div>
                            <input id="dawimg-gain-input-${sk}" type="text" value="${dawImgFmtGain(s.gain)}" onchange="window.dawImagerSetFieldFromText('${key}','gain', this.value)" class="w-14 bg-black/50 border border-[rgba(47,208,255,0.4)] rounded px-1 py-1 text-[8px] text-center neon-blue-text font-bold outline-none">
                        </div>
                        <div class="flex flex-col items-center gap-1.5">
                    ...
                    <div class="rounded-md px-2 py-2 bg-black/40" style="border:1px solid rgba(47,208,255,0.25);">
                        <div class="text-[6.5px] text-gray-500 uppercase font-black tracking-widest text-center mb-1.5">Input Mode</div>
                        <div class="flex rounded overflow-hidden border border-[rgba(47,208,255,0.4)]" id="dawimg-inputmode-${sk}">
                            <button onclick="window.dawImagerToggleMode('${key}','inputMode','LR')" class="flex-1 text-[8px] font-black py-1 transition-colors ${s.inputMode === 'LR' ? 'bg-[#2fd0ff] text-black' : 'text-gray-500 hover:text-[#2fd0ff]'}">L-R</button>
                            <button onclick="window.dawImagerToggleMode('${key}','inputMode','MS')" class="flex-1 text-[8px] font-black py-1 transition-colors ${s.inputMode === 'MS' ? 'bg-[#2fd0ff] text-black' : 'text-gray-500 hover:text-[#2fd0ff]'}">M-S</button>
                        </div>
                        <div class="flex items-center justify-center gap-1.5 mt-2" id="dawimg-phase-${sk}">
                            <button onclick="window.dawImagerTogglePhase('${key}','invL')" title="Invert L phase" class="w-6 h-6 rounded text-[9px] font-black border transition-colors ${s.invL ? 'bg-[#2fd0ff] text-black border-[#2fd0ff]' : 'text-gray-400 border-[rgba(47,208,255,0.35)] hover:text-[#2fd0ff]'}">+</button>
                            <button onclick="window.dawImagerTogglePhase('${key}','swap')" title="Swap L/R" class="w-8 h-6 rounded text-[9px] font-black border transition-colors ${s.swap ? 'bg-[#2fd0ff] text-black border-[#2fd0ff]' : 'text-gray-400 border-[rgba(47,208,255,0.35)] hover:text-[#2fd0ff]'}">&#8596;</button>
                            <button onclick="window.dawImagerTogglePhase('${key}','invR')" title="Invert R phase" class="w-6 h-6 rounded text-[9px] font-black border transition-colors ${s.invR ? 'bg-[#2fd0ff] text-black border-[#2fd0ff]' : 'text-gray-400 border-[rgba(47,208,255,0.35)] hover:text-[#2fd0ff]'}">+</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex-1 min-w-0 rounded-lg overflow-hidden" style="border:1px solid rgba(47,208,255,0.35); box-shadow: inset 0 0 12px rgba(47,208,255,0.08);">
                    <svg id="dawimg-goniometer-${sk}" viewBox="0 0 ${DAW_IMG_GRAPH_W} ${DAW_IMG_GRAPH_H}" style="width:100%; height:190px; display:block;">${dawImagerGoniometerInner(key, s, 0)}</svg>
                </div>

                <!-- RIGHT: Output meters + Meter Mode -->
                <div class="flex flex-col gap-2 flex-shrink-0 items-center">
                    <span class="text-[7px] text-gray-500 uppercase font-black tracking-widest">Output</span>
                    <div class="flex-1 min-h-0 rounded-lg flex items-stretch justify-center gap-3 py-2 px-2" style="background:#000; border:1px solid rgba(47,208,255,0.35); box-shadow: inset 0 0 12px rgba(47,208,255,0.08);">
                        <div class="flex flex-col items-center gap-1.5">
                            <span class="text-[7px] text-gray-500 uppercase font-black tracking-widest">L</span>
                            <div class="daw-led-meter-single" id="dawimg-led-L-${sk}" style="height:100px;"><div class="daw-led-mask" style="height:100%;"></div></div>
                            <span id="dawimg-peak-L-${sk}" class="text-[7px] neon-blue-text font-bold">-Inf</span>
                        </div>
                        <div class="flex flex-col items-center gap-1.5">
                            <span class="text-[7px] text-gray-500 uppercase font-black tracking-widest">R</span>
                            <div class="daw-led-meter-single" id="dawimg-led-R-${sk}" style="height:100px;"><div class="daw-led-mask" style="height:100%;"></div></div>
                            <span id="dawimg-peak-R-${sk}" class="text-[7px] neon-blue-text font-bold">-Inf</span>
                        </div>
                    </div>
                    <div class="rounded-md px-1.5 py-1.5 bg-black/40 w-full" style="border:1px solid rgba(47,208,255,0.25);">
                        <div class="text-[6.5px] text-gray-500 uppercase font-black tracking-widest text-center mb-1">Meter Mode</div>
                        <div class="flex rounded overflow-hidden border border-[rgba(47,208,255,0.4)]" id="dawimg-metermode-${sk}">
                            <button onclick="window.dawImagerToggleMode('${key}','meterMode','LR')" class="flex-1 text-[7px] font-black py-1 transition-colors ${s.meterMode === 'LR' ? 'bg-[#2fd0ff] text-black' : 'text-gray-500 hover:text-[#2fd0ff]'}">L-R</button>
                            <button onclick="window.dawImagerToggleMode('${key}','meterMode','MS')" class="flex-1 text-[7px] font-black py-1 transition-colors ${s.meterMode === 'MS' ? 'bg-[#2fd0ff] text-black' : 'text-gray-500 hover:text-[#2fd0ff]'}">M-S</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2 mt-3">
                <span class="text-[8px] text-gray-500 uppercase font-black tracking-widest w-16 flex-shrink-0">Asymmetry</span>
                <input id="dawimg-asym-slider-${sk}" type="range" min="-90" max="90" step="1" value="${s.asymmetry}" class="eq8-hslider"
                    oninput="window.dawImagerSetField('${key}','asymmetry', this.value)">
                <input id="dawimg-asym-input-${sk}" type="text" value="${Math.round(s.asymmetry)}" onchange="window.dawImagerSetFieldFromText('${key}','asymmetry', this.value)" class="w-14 bg-black/50 border border-[rgba(47,208,255,0.4)] rounded px-1.5 py-1 text-[8px] text-center neon-blue-text font-bold flex-shrink-0 outline-none">
            </div>
            <div class="flex items-center gap-2 mt-2.5">
                <span class="text-[8px] text-gray-500 uppercase font-black tracking-widest w-16 flex-shrink-0">Rotation</span>
                <input id="dawimg-rot-slider-${sk}" type="range" min="-45" max="45" step="0.1" value="${s.rotation}" class="eq8-hslider"
                    oninput="window.dawImagerSetField('${key}','rotation', this.value)">
                <input id="dawimg-rot-input-${sk}" type="text" value="${s.rotation.toFixed(1)}" onchange="window.dawImagerSetFieldFromText('${key}','rotation', this.value)" class="w-14 bg-black/50 border border-[rgba(47,208,255,0.4)] rounded px-1.5 py-1 text-[8px] text-center neon-blue-text font-bold flex-shrink-0 outline-none">
            </div>`;
        };

        window.dawImagerSetField = function(key, field, value) {
            const s = dawImagerGetState(key);
            s[field] = parseFloat(value);
            delete window.dawFxActivePresetLabel[key];
            const sk = eq8SafeKey(key);
            const badge = document.getElementById(`dawimg-badge-${sk}`);
            if (badge) badge.innerText = 'factory preset';
            if (field === 'gain') { const inp = document.getElementById(`dawimg-gain-input-${sk}`); if (inp) inp.value = dawImgFmtGain(s.gain); }
            else if (field === 'width') { const inp = document.getElementById(`dawimg-width-input-${sk}`); if (inp) inp.value = s.width.toFixed(2); }
            else if (field === 'asymmetry') { const inp = document.getElementById(`dawimg-asym-input-${sk}`); if (inp) inp.value = Math.round(s.asymmetry); }
            else if (field === 'rotation') { const inp = document.getElementById(`dawimg-rot-input-${sk}`); if (inp) inp.value = s.rotation.toFixed(1); }
        };

        window.dawImagerSetFieldFromText = function(key, field, text) {
            const num = parseFloat(String(text).replace(/[^0-9.\-]/g, ''));
            if (isNaN(num)) return;
            window.dawImagerSetField(key, field, num);
            const sk = eq8SafeKey(key);
            const s = dawImagerGetState(key);
            if (field === 'gain') { const sl = document.getElementById(`dawimg-gain-slider-${sk}`); if (sl) sl.value = s.gain; }
            if (field === 'width') { const sl = document.getElementById(`dawimg-width-slider-${sk}`); if (sl) sl.value = s.width; }
            if (field === 'asymmetry') { const sl = document.getElementById(`dawimg-asym-slider-${sk}`); if (sl) sl.value = s.asymmetry; }
            if (field === 'rotation') { const sl = document.getElementById(`dawimg-rot-slider-${sk}`); if (sl) sl.value = s.rotation; }
        };

        window.dawImagerToggleMode = function(key, whichField, value) {
            const s = dawImagerGetState(key);
            s[whichField] = value;
            const sk = eq8SafeKey(key);
            const wrap = document.getElementById(whichField === 'inputMode' ? `dawimg-inputmode-${sk}` : `dawimg-metermode-${sk}`);
            if (wrap) {
                const btns = wrap.querySelectorAll('button');
                btns.forEach(b => {
                    const isOn = b.getAttribute('onclick').includes(`'${value}'`);
                    b.classList.toggle('bg-[#2fd0ff]', isOn);
                    b.classList.toggle('text-black', isOn);
                    b.classList.toggle('text-gray-500', !isOn);
                });
            }
        };

        window.dawImagerTogglePhase = function(key, which) {
            const s = dawImagerGetState(key);
            s[which] = !s[which];
            const sk = eq8SafeKey(key);
            const wrap = document.getElementById(`dawimg-phase-${sk}`);
            if (wrap) {
                const btn = which === 'invL' ? wrap.children[0] : which === 'swap' ? wrap.children[1] : wrap.children[2];
                if (btn) {
                    btn.classList.toggle('bg-[#2fd0ff]', s[which]);
                    btn.classList.toggle('text-black', s[which]);
                    btn.classList.toggle('border-[#2fd0ff]', s[which]);
                    btn.classList.toggle('text-gray-400', !s[which]);
                }
            }
        };

        window.dawImagerTogglePresetMenu = function(key) {
            const sk = eq8SafeKey(key);
            const el = document.getElementById(`dawimg-preset-menu-${sk}`);
            if (!el) return;
            const isHidden = el.classList.contains('hidden-section');
            if (isHidden) { window.dawImagerRenderPresetMenu(key); el.classList.remove('hidden-section'); }
            else el.classList.add('hidden-section');
        };

        window.dawImagerRenderPresetMenu = function(key) {
            const sk = eq8SafeKey(key);
            const el = document.getElementById(`dawimg-preset-menu-${sk}`);
            if (!el) return;
            const userPresets = (window.dawFxUserPresets['stereo-imager'] || []);
            const activeLabel = window.dawFxActivePresetLabel[key];
            el.innerHTML = `
            <div class="absolute top-full right-0 mt-1.5 w-56 z-20 bg-black rounded-lg overflow-y-auto slick-scroll" style="border:1px solid #2fd0ff; box-shadow: 0 0 16px rgba(47,208,255,0.4), inset 0 0 2px rgba(47,208,255,0.45); max-height:220px;" onclick="event.stopPropagation()">
                <button onclick="window.dawImagerApplyPreset('${key}', null)" class="w-full text-left px-3 py-2 text-[9px] font-black uppercase tracking-widest neon-blue-text hover:bg-[#2fd0ff]/15 transition-colors border-b border-[rgba(47,208,255,0.25)]">Reset to factory default</button>
                <button onclick="window.dawImagerApplyPreset('${key}', null)" class="w-full flex items-center gap-2 text-left px-3 py-2 text-[9px] font-black uppercase tracking-widest transition-colors border-b border-[rgba(47,208,255,0.15)] ${!activeLabel ? 'bg-[#2fd0ff]/20 neon-blue-text' : 'text-gray-400 hover:bg-white/5'}">
                    <span class="w-3 flex-shrink-0">${!activeLabel ? '✓' : ''}</span> No preset
                </button>
                ${userPresets.length ? `<div class="px-3 py-1.5 text-[7px] text-gray-500 uppercase tracking-[0.2em] border-b border-[rgba(47,208,255,0.15)]">---- User Presets ----</div>` : ''}
                ${userPresets.map((p, i) => `
                <button onclick="window.dawImagerApplyPreset('${key}', ${i})" class="w-full flex items-center gap-2 text-left px-3 py-2 text-[9px] font-bold tracking-wide transition-colors ${activeLabel === p.name ? 'bg-[#2fd0ff]/20 neon-blue-text' : 'text-gray-300 hover:bg-white/5 hover:text-[#2fd0ff]'}">
                    <span class="w-3 flex-shrink-0">${activeLabel === p.name ? '✓' : ''}</span> <span class="truncate">${p.name}</span>
                </button>`).join('')}
            </div>`;
        };

        window.dawImagerApplyPreset = function(key, presetIndex) {
            if (presetIndex === null || presetIndex === undefined) {
                window.dawImagerState[key] = dawImagerDefaultState();
                delete window.dawFxActivePresetLabel[key];
            } else {
                const preset = (window.dawFxUserPresets['stereo-imager'] || [])[presetIndex];
                if (preset) {
                    window.dawImagerState[key] = JSON.parse(JSON.stringify(preset.data));
                    window.dawFxActivePresetLabel[key] = preset.name;
                }
            }
            window.renderDawFxPicker();
        ...
        // Live goniometer animation — driven by window.requestAnimationFrame.
        // Stops itself automatically once the plugin panel is no longer in
        // the DOM (e.g. the picker was closed or another plugin selected).
        window.dawImagerRafIds = window.dawImagerRafIds || {};
        window.dawImagerAnimate = function(key) {
            const sk = eq8SafeKey(key);
            if (window.dawImagerRafIds[sk]) return; // loop already running for this instance
            const step = (t) => {
                const svg = document.getElementById(`dawimg-goniometer-${sk}`);
                if (!svg) { delete window.dawImagerRafIds[sk]; return; }
                const s = dawImagerGetState(key);
                const jitter = Math.sin(t / 180) * 5 + Math.sin(t / 71) * 2.5;
                svg.innerHTML = dawImagerGoniometerInner(key, s, jitter);
                window.dawImagerRafIds[sk] = window.requestAnimationFrame(step);
            };
            window.dawImagerRafIds[sk] = window.requestAnimationFrame(step);
        };

        // ============================================================
        // BASS MAXIMIZER — dual sub-band engine: SUB 1 / SUB 2 (gain
        // knob + editable freq LCD + attack/decay or width/smooth
        // trim knobs), TONE (drive knob + Tube toggle), STABILISE
        // (knob + live rAF-animated level bars), an HP/LP crossover
        // strip, Dry/Wet, Bypass, and IN/OUT trim faders — topped by
        // an animated spectrum header (also driven by requestAnimationFrame).
        // ============================================================
        window.dawBmState = window.dawBmState || {}; // key -> full param state

        function dawBmDefaultState() {
            return {
                sub1On: true, sub1Gain: 0, sub1Freq: 45, sub1Attack: 20, sub1Decay: 60,
                sub2On: true, sub2Gain: 0, sub2Freq: 100, sub2Width: 40, sub2Smooth: 30,
                tone: 0, tube: true,
                stabilise: 50,
                hp: 40, lp: 12000,
                dry: 100, wet: 100,
        ...
        function dawBmGetState(key) {
            if (!window.dawBmState[key]) window.dawBmState[key] = dawBmDefaultState();
            return window.dawBmState[key];
        }

        const DAW_BM_FIELDS = {
            sub1Gain:  { min: -60, max: 12,    unit: 'd',  decimals: 1, signed: true },
            sub2Gain:  { min: -60, max: 12,    unit: 'd',  decimals: 1, signed: true },
            inGain:    { min: -24, max: 12,    unit: 'd',  decimals: 1, signed: true },
            outGain:   { min: -24, max: 12,    unit: 'd',  decimals: 1, signed: true },
            sub1Attack:{ min: 1,   max: 200,   unit: 'ms', decimals: 0 },
            sub1Decay: { min: 1,   max: 500,   unit: 'ms', decimals: 0 },
            sub2Width: { min: 0,   max: 100,   unit: '%',  decimals: 0 },
            sub2Smooth:{ min: 0,   max: 100,   unit: '%',  decimals: 0 },
            tone:      { min: -100,max: 100,   unit: '',   decimals: 0, signed: true },
            stabilise: { min: 0,   max: 100,   unit: '%',  decimals: 0 },
            dry:       { min: 0,   max: 100,   unit: '%',  decimals: 0 },
            wet:       { min: 0,   max: 100,   unit: '%',  decimals: 0 },
            sub1Freq:  { min: 20,  max: 250,   unit: 'Hz', decimals: 0 },
            sub2Freq:  { min: 40,  max: 400,   unit: 'Hz', decimals: 0 },
            hp:        { min: 20,  max: 500,   unit: 'Hz', decimals: 0 },
            lp:        { min: 2000,max: 20000, unit: 'Hz', decimals: 0 }
        };
        function dawBmFmt(field, v) {
            const c = DAW_BM_FIELDS[field];
            let s = v.toFixed(c.decimals);
            if (c.signed && v >= 0) s = '+' + s;
            return s + (c.unit ? (c.unit === 'd' ? '' : ' ' + c.unit) : '');
        }

        function dawBmKnob(key, sk, field, label, size) {
            const s = dawBmGetState(key);
            const c = DAW_BM_FIELDS[field];
            ...
            <div class="flex flex-col items-center gap-1">
                ...
                <div id="dawbm-knob-${field}-${sk}" class="relative rounded-full bg-black border-2 border-[rgba(47,208,255,0.45)] cursor-ns-resize select-none flex-shrink-0"
                     style="width:${dim}px; height:${dim}px;"
                     onmousedown="event.stopPropagation(); window.dawBmKnobDrag(event,'${key}','${field}')" ontouchstart="event.stopPropagation(); window.dawBmKnobDrag(event,'${key}','${field}')">
                    <div id="dawbm-knob-ind-${field}-${sk}" class="absolute top-0.5 left-1/2 w-0.5 bg-[#2fd0ff] origin-bottom" style="height:${dim * 0.4}px; transform:translateX(-50%) rotate(${deg}deg);"></div>
                </div>
                <input id="dawbm-val-${field}-${sk}" type="text" value="${dawBmFmt(field, s[field])}" onclick="event.stopPropagation()" onchange="window.dawBmSetFromText('${key}','${field}', this.value)" class="w-12 bg-black/50 border border-[rgba(47,208,255,0.35)] rounded px-1 py-0.5 text-[7px] text-center neon-blue-text font-bold outline-none">
            </div>`;
        }

        window.dawBmKnobDrag = function(e, key, field) {
            e.preventDefault();
            const c = DAW_BM_FIELDS[field];
            const s = dawBmGetState(key);
            const startY = e.touches ? e.touches[0].clientY : e.clientY;
            ...
            const move = (ev) => {
                ...
                s[field] = newVal;
                window.dawBmUpdateKnobVisual(key, field);
            };
            const up = () => {
                document.removeEventListener('mousemove', move);
                document.removeEventListener('mouseup', up);
        ...
        window.dawBmUpdateKnobVisual = function(key, field) {
            const sk = eq8SafeKey(key);
            const s = dawBmGetState(key);
            const c = DAW_BM_FIELDS[field];
            const pct = (s[field] - c.min) / (c.max - c.min);
            const deg = -135 + pct * 270;
            delete window.dawFxActivePresetLabel[key];
            const badge = document.getElementById(`dawbm-badge-${sk}`);
            if (badge) badge.innerText = 'factory preset';
            const ind = document.getElementById(`dawbm-knob-ind-${field}-${sk}`);
            if (ind) ind.style.transform = `translateX(-50%) rotate(${deg}deg)`;
            const val = document.getElementById(`dawbm-val-${field}-${sk}`);
            if (val) val.value = dawBmFmt(field, s[field]);
            if (field === 'sub1Gain' || field === 'sub1Freq') window.dawBmRedrawSpectrum(key);
            if (field === 'sub2Gain' || field === 'sub2Freq') window.dawBmRedrawSpectrum(key);
        };

        window.dawBmSetFromText = function(key, field, text) {
            const c = DAW_BM_FIELDS[field];
            const num = parseFloat(String(text).replace(/[^0-9.\-]/g, ''));
            if (isNaN(num)) return;
            const s = dawBmGetState(key);
        ...
        window.dawBmToggle = function(key, field) {
            const s = dawBmGetState(key);
            s[field] = !s[field];
            const sk = eq8SafeKey(key);
            delete window.dawFxActivePresetLabel[key];
        ...
        function dawBmSliderRow(key, sk, field, label, fmtOverride) {
            const s = dawBmGetState(key);
            const c = DAW_BM_FIELDS[field];
            const val = fmtOverride ? fmtOverride(s[field]) : dawBmFmt(field, s[field]);
            return `
            <div class="flex items-center gap-2">
                <span class="text-[8px] text-gray-500 uppercase font-black tracking-widest w-8 flex-shrink-0">${label}</span>
                <input id="dawbm-slider-${field}-${sk}" type="range" min="${c.min}" max="${c.max}" step="1" value="${s[field]}" class="eq8-hslider"
                    oninput="window.dawBmSetSlider('${key}','${field}', this.value)">
                <input id="dawbm-sliderval-${field}-${sk}" type="text" value="${val}" onchange="window.dawBmSetFromText('${key}','${field}', this.value)" class="w-16 bg-black/50 border border-[rgba(47,208,255,0.4)] rounded px-1.5 py-1 text-[8px] text-center neon-blue-text font-bold flex-shrink-0 outline-none">
            </div>`;
        }
        window.dawBmSetSlider = function(key, field, value) {
            window.dawBmSetFromText(key, field, value);
            const sk = eq8SafeKey(key);
            const el = document.getElementById(`dawbm-sliderval-${field}-${sk}`);
            const s = dawBmGetState(key);
            if (el) el.value = field === 'lp' ? (s.lp >= 1000 ? (s.lp / 1000).toFixed(1) + ' kHz' : Math.round(s.lp) + ' Hz') : dawBmFmt(field, s[field]);
        };

        // --- Spectrum header: two humps (sub1=blue, sub2=orange) whose
        // position/height track the live freq & gain knobs, plus a
        // subtle animated noise-floor silhouette. Redrawn continuously
        // via window.requestAnimationFrame while the panel is open.
        const DAW_BM_SPEC_W = 640, DAW_BM_SPEC_H = 120;
        function dawBmHumpPath(cx, height, widthPx) {
            const pts = [];
            const N = 40;
            for (let i = 0; i <= N; i++) {
                const x = cx - widthPx + (i / N) * widthPx * 2;
                const t = (x - cx) / widthPx;
                const y = DAW_BM_SPEC_H - 4 - height * Math.exp(-t * t * 4);
                pts.push(`${x.toFixed(1)},${Math.max(4, y).toFixed(1)}`);
            }
            return `${cx - widthPx},${DAW_BM_SPEC_H - 4} ` + pts.join(' ') + ` ${cx + widthPx},${DAW_BM_SPEC_H - 4}`;
        }
        function dawBmSpectrumInner(key, s, t) {
            const cx1 = (eq8FreqToX(Math.max(20, s.sub1Freq)) / EQ8_GRAPH_W) * DAW_BM_SPEC_W;
            const cx2 = (eq8FreqToX(Math.max(20, s.sub2Freq)) / EQ8_GRAPH_W) * DAW_BM_SPEC_W;
            const h1 = 14 + ((s.sub1Gain + 60) / 72) * 74;
            const h2 = 14 + ((s.sub2Gain + 60) / 72) * 74;
            let noise = 'M 0 ' + (DAW_BM_SPEC_H - 6);
            const bars = 60;
            for (let i = 0; i <= bars; i++) {
                const x = (i / bars) * DAW_BM_SPEC_W;
                const n = Math.abs(Math.sin(i * 12.9898 + t / 400)) * Math.abs(Math.cos(i * 4.14 + t / 900));
                const y = DAW_BM_SPEC_H - 6 - n * 44 * (0.5 + i / bars);
                noise += ` L ${x.toFixed(1)} ${y.toFixed(1)}`;
            }
            return `
                <rect x="0" y="0" width="${DAW_BM_SPEC_W}" height="${DAW_BM_SPEC_H}" fill="#000"/>
                <path d="${noise}" fill="none" stroke="rgba(47,208,255,0.18)" stroke-width="1"/>
                <polygon points="${dawBmHumpPath(cx1, h1, 90)}" fill="rgba(47,208,255,0.30)" stroke="#2fd0ff" stroke-width="1.4"/>
                <polygon points="${dawBmHumpPath(cx2, h2, 70)}" fill="rgba(255,140,40,0.28)" stroke="#ff9a3d" stroke-width="1.4"/>
                <text x="${DAW_BM_SPEC_W / 2}" y="20" font-size="15" font-weight="900" letter-spacing="4" fill="#2fd0ff" text-anchor="middle" opacity="0.9">BASS MAXIMIZER</text>`;
        }
        window.dawBmRedrawSpectrum = function(key) {
            const sk = eq8SafeKey(key);
            const svg = document.getElementById(`dawbm-spectrum-${sk}`);
            if (!svg) return;
            svg.innerHTML = dawBmSpectrumInner(key, dawBmGetState(key), performance.now());
        };

        window.dawBmRafIds = window.dawBmRafIds || {};
        window.dawBmAnimate = function(key) {
            const sk = eq8SafeKey(key);
            if (window.dawBmRafIds[sk]) return;
            const step = (t) => {
                const svg = document.getElementById(`dawbm-spectrum-${sk}`);
                if (!svg) { delete window.dawBmRafIds[sk]; return; }
                svg.innerHTML = dawBmSpectrumInner(key, dawBmGetState(key), t);
                const bars = document.getElementById(`dawbm-stabilise-bars-${sk}`);
                if (bars) {
                    const s = dawBmGetState(key);
                    const amt = s.stabilise / 100;
                    const heights = Array.from({ length: 10 }, (_, i) =>
                        (0.25 + amt * 0.7 * Math.abs(Math.sin(t / (150 + i * 17) + i))) * 100);
                    bars.innerHTML = heights.map(h => `<div style="width:2px; height:${h.toFixed(0)}%; background:#a78bfa; box-shadow:0 0 4px rgba(167,139,250,0.7); border-radius:1px;"></div>`).join('');
                }
                window.dawBmRafIds[sk] = window.requestAnimationFrame(step);
            };
            window.dawBmRafIds[sk] = window.requestAnimationFrame(step);
        };

        window.dawBmPanel = function(key, plugin) {
            const s = dawBmGetState(key);
            const sk = eq8SafeKey(key);
            try { window.dawBmAnimate(key); } catch (e) {}
            const onOffBtn = (field, sizeCls) => `
                <button onclick="event.stopPropagation(); window.dawBmToggle('${key}','${field}')" class="${sizeCls} rounded-full flex items-center justify-center border transition-colors ${s[field] ? 'bg-[#2fd0ff] border-[#2fd0ff] text-black' : 'border-[rgba(47,208,255,0.4)] text-gray-500 hover:text-[#2fd0ff]'}" title="On/Off">
                    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4"><path d="M12 2v8"/><circle cx="12" cy="14" r="7"/></svg>
                </button>`;
            const faderMini = (field, label) => `
                <div class="flex flex-col items-center gap-1">
                    <div class="daw-fader-track" style="height:110px;">
                        <input id="dawbm-fader-${field}-${sk}" type="range" min="${DAW_BM_FIELDS[field].min}" max="${DAW_BM_FIELDS[field].max}" step="0.1" value="${s[field]}" class="daw-fader-input eq8-fader-thumb"
                            oninput="window.dawBmSetFromText('${key}','${field}', this.value)">
                    </div>
                    <input id="dawbm-val-${field}-${sk}" type="text" value="${dawBmFmt(field, s[field])}" onchange="window.dawBmSetFromText('${key}','${field}', this.value)" class="w-12 bg-black/50 border border-[rgba(47,208,255,0.35)] rounded px-1 py-0.5 text-[7px] text-center neon-blue-text font-bold outline-none">
                    <span class="text-[6.5px] text-gray-500 uppercase font-black tracking-widest">${label}</span>
                </div>`;
            ...
            <div class="flex items-start justify-between gap-2 mb-1">
                ...
                <span class="relative inline-block flex-shrink-0">
                    <button onclick="event.stopPropagation(); window.dawBmTogglePresetMenu('${key}')" class="flex items-center gap-1 text-[8px] font-black uppercase tracking-widest neon-blue-text border border-[rgba(47,208,255,0.5)] rounded px-2 py-1 bg-black/50 hover:bg-[#2fd0ff]/15 transition-colors max-w-[150px]">
                        <span id="dawbm-badge-${sk}" class="truncate">${window.dawFxActivePresetLabel[key] || 'factory preset'}</span>
                        <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" class="flex-shrink-0"><path d="M6 9l6 6 6-6"/></svg>
                    </button>
                    <div id="dawbm-preset-menu-${sk}" class="hidden-section"></div>
                </span>
            </div>
            <div class="inline-block text-[8px] neon-blue-text uppercase font-black tracking-widest mt-1 border border-[rgba(47,208,255,0.35)] rounded px-1.5 py-0.5">${plugin.category}</div>

            <div class="rounded-lg overflow-hidden mt-3" style="border:1px solid rgba(47,208,255,0.35); box-shadow: inset 0 0 12px rgba(47,208,255,0.08);">
                <svg id="dawbm-spectrum-${sk}" viewBox="0 0 ${DAW_BM_SPEC_W} ${DAW_BM_SPEC_H}" style="width:100%; height:110px; display:block;">${dawBmSpectrumInner(key, s, 0)}</svg>
            </div>
            ...
            <div class="flex gap-2 mt-3">
                ...
                <div class="flex-1 grid grid-cols-4 gap-2">
                    <div class="rounded-lg bg-black/40 p-2" style="border:1px solid rgba(47,208,255,0.25);">
                        <div class="flex items-center justify-between mb-1.5">
                            <span class="neon-blue-text text-[9px] font-black uppercase tracking-widest">Sub 1</span>
                            ${onOffBtn('sub1On', 'w-4 h-4')}
                        </div>
                        <div class="flex justify-center">${dawBmKnob(key, sk, 'sub1Gain', 'Gain', 40)}</div>
                        <div class="rounded px-1.5 py-1 mt-1.5 text-center" style="background:rgba(47,208,255,0.08); border:1px solid rgba(47,208,255,0.4);">
                            <div class="text-[6px] text-gray-500 uppercase font-black tracking-widest">Freq</div>
                            <input id="dawbm-val-sub1Freq-${sk}" type="text" value="${dawBmFmt('sub1Freq', s.sub1Freq)}" onchange="window.dawBmSetFromText('${key}','sub1Freq', this.value)" class="w-full bg-transparent text-center text-[13px] font-black neon-blue-text outline-none">
                        </div>
                        <div class="flex justify-center gap-2 mt-1.5">
                            ${dawBmKnob(key, sk, 'sub1Attack', 'Attack', 24)}
                            ${dawBmKnob(key, sk, 'sub1Decay', 'Decay', 24)}
                        </div>
                    </div>

                    <div class="rounded-lg bg-black/40 p-2" style="border:1px solid rgba(255,154,61,0.25);">
                        <div class="flex items-center justify-between mb-1.5">
                            <span class="text-[9px] font-black uppercase tracking-widest" style="color:#ff9a3d;">Sub 2</span>
                            ${onOffBtn('sub2On', 'w-4 h-4')}
                        </div>
                        <div class="flex justify-center">${dawBmKnob(key, sk, 'sub2Gain', 'Gain', 40)}</div>
                        <div class="rounded px-1.5 py-1 mt-1.5 text-center" style="background:rgba(255,154,61,0.08); border:1px solid rgba(255,154,61,0.4);">
                            <div class="text-[6px] text-gray-500 uppercase font-black tracking-widest">Freq</div>
                            <input id="dawbm-val-sub2Freq-${sk}" type="text" value="${dawBmFmt('sub2Freq', s.sub2Freq)}" onchange="window.dawBmSetFromText('${key}','sub2Freq', this.value)" class="w-full bg-transparent text-center text-[13px] font-black outline-none" style="color:#ff9a3d;">
                        </div>
                        <div class="flex justify-center gap-2 mt-1.5">
                            ${dawBmKnob(key, sk, 'sub2Width', 'Width', 24)}
                            ${dawBmKnob(key, sk, 'sub2Smooth', 'Smooth', 24)}
                        </div>
                    </div>

                    <div class="rounded-lg bg-black/40 p-2 flex flex-col items-center" style="border:1px solid rgba(47,208,255,0.25);">
                        <div class="flex items-center justify-between w-full mb-1.5">
                            <span class="neon-blue-text text-[9px] font-black uppercase tracking-widest">Tone</span>
                            <span class="w-4 h-4"></span>
                        </div>
                        ${dawBmKnob(key, sk, 'tone', 'Tone', 40)}
                        <button onclick="window.dawBmToggle('${key}','tube')" class="mt-2 w-full rounded py-1.5 text-[8px] font-black uppercase tracking-widest border transition-colors ${s.tube ? 'text-black' : 'text-gray-500 border-[rgba(255,154,61,0.35)] hover:text-[#ff9a3d]'}" style="${s.tube ? 'background:#ff9a3d; border-color:#ff9a3d; box-shadow:0 0 10px rgba(255,154,61,0.6);' : ''}">Tube</button>
                    </div>

                    <div class="rounded-lg bg-black/40 p-2 flex flex-col items-center" style="border:1px solid rgba(167,139,250,0.3);">
                        <span class="text-[9px] font-black uppercase tracking-widest mb-1.5" style="color:#a78bfa;">Stabilise</span>
                        ${dawBmKnob(key, sk, 'stabilise', 'Amt', 40)}
                        <div id="dawbm-stabilise-bars-${sk}" class="flex items-end gap-0.5 mt-2" style="height:26px;"></div>
                    </div>
                </div>

                ${faderMini('outGain', 'Out')}
            </div>

            <div class="flex gap-4 mt-3">
                <div class="flex-1 space-y-2">
                    ${dawBmSliderRow(key, sk, 'hp', 'HP')}
                    ${dawBmSliderRow(key, sk, 'lp', 'LP', v => v >= 1000 ? (v / 1000).toFixed(1) + ' kHz' : Math.round(v) + ' Hz')}
                </div>
                <div class="flex items-end gap-3 flex-shrink-0">
                    ${dawBmKnob(key, sk, 'dry', 'Dry', 30)}
                    ${dawBmKnob(key, sk, 'wet', 'Wet', 30)}
                    <button onclick="window.dawBmToggle('${key}','bypass')" class="h-[30px] rounded px-3 text-[8px] font-black uppercase tracking-widest border transition-colors ${s.bypass ? 'bg-red-500 border-red-500 text-black' : 'text-gray-500 border-[rgba(47,208,255,0.35)] hover:text-[#2fd0ff]'}">Bypass</button>
                </div>
            </div>`;
        };

        window.dawBmTogglePresetMenu = function(key) {
            const sk = eq8SafeKey(key);
            const el = document.getElementById(`dawbm-preset-menu-${sk}`);
            if (!el) return;
            const isHidden = el.classList.contains('hidden-section');
            if (isHidden) { window.dawBmRenderPresetMenu(key); el.classList.remove('hidden-section'); }
            else el.classList.add('hidden-section');
        };

        window.dawBmRenderPresetMenu = function(key) {
            const sk = eq8SafeKey(key);
            const el = document.getElementById(`dawbm-preset-menu-${sk}`);
            if (!el) return;
            const userPresets = (window.dawFxUserPresets['bass-maximizer'] || []);
            const activeLabel = window.dawFxActivePresetLabel[key];
            el.innerHTML = `
            <div class="absolute top-full right-0 mt-1.5 w-56 z-20 bg-black rounded-lg overflow-y-auto slick-scroll" style="border:1px solid #2fd0ff; box-shadow: 0 0 16px rgba(47,208,255,0.4), inset 0 0 2px rgba(47,208,255,0.45); max-height:220px;" onclick="event.stopPropagation()">
                <button onclick="window.dawBmApplyPreset('${key}', null)" class="w-full text-left px-3 py-2 text-[9px] font-black uppercase tracking-widest neon-blue-text hover:bg-[#2fd0ff]/15 transition-colors border-b border-[rgba(47,208,255,0.25)]">Reset to factory default</button>
                <button onclick="window.dawBmApplyPreset('${key}', null)" class="w-full flex items-center gap-2 text-left px-3 py-2 text-[9px] font-black uppercase tracking-widest transition-colors border-b border-[rgba(47,208,255,0.15)] ${!activeLabel ? 'bg-[#2fd0ff]/20 neon-blue-text' : 'text-gray-400 hover:bg-white/5'}">
                    <span class="w-3 flex-shrink-0">${!activeLabel ? '✓' : ''}</span> No preset
                </button>
                ${userPresets.length ? `<div class="px-3 py-1.5 text-[7px] text-gray-500 uppercase tracking-[0.2em] border-b border-[rgba(47,208,255,0.15)]">---- User Presets ----</div>` : ''}
                ${userPresets.map((p, i) => `
                <button onclick="window.dawBmApplyPreset('${key}', ${i})" class="w-full flex items-center gap-2 text-left px-3 py-2 text-[9px] font-bold tracking-wide transition-colors ${activeLabel === p.name ? 'bg-[#2fd0ff]/20 neon-blue-text' : 'text-gray-300 hover:bg-white/5 hover:text-[#2fd0ff]'}">
                    <span class="w-3 flex-shrink-0">${activeLabel === p.name ? '✓' : ''}</span> <span class="truncate">${p.name}</span>
                </button>`).join('')}
            </div>`;
        };

        window.dawBmApplyPreset = function(key, presetIndex) {
            if (presetIndex === null || presetIndex === undefined) {
                window.dawBmState[key] = dawBmDefaultState();
                delete window.dawFxActivePresetLabel[key];
            } else {
                const preset = (window.dawFxUserPresets['bass-maximizer'] || [])[presetIndex];
                if (preset) {
                    window.dawBmState[key] = JSON.parse(JSON.stringify(preset.data));
                    window.dawFxActivePresetLabel[key] = preset.name;
                }
            }
            window.renderDawFxPicker();
        ...
        // ============================================================
        // VOC RIDE — a "VOCRIDE"-style formant/harmonic EQ panel: a log-
        // frequency response graph with 4 colour-coded draggable nodes
        // (1st/2nd/3rd formant + an "Nth pos" band), a faint decorative
        // spectrum trace behind it, and a knob row (1st/2nd/3rd/Nth pos/
        // Gain/Q/Out vol) whose ring colours match their nodes. Animated
        // live via window.requestAnimationFrame while the panel is open.
        // ============================================================
        window.dawVrState = window.dawVrState || {};

        const DAW_VR_GRAPH_W = 640, DAW_VR_GRAPH_H = 190;
        const DAW_VR_FREQ_MIN = 20, DAW_VR_FREQ_MAX = 20000;
        const DAW_VR_DB_RANGE = 18; // +/- dB shown on the graph

        function dawVrFreqToX(f) { return ((Math.log10(f) - Math.log10(DAW_VR_FREQ_MIN)) / (Math.log10(DAW_VR_FREQ_MAX) - Math.log10(DAW_VR_FREQ_MIN))) * DAW_VR_GRAPH_W; }
        function dawVrXToFreq(x) { return Math.pow(10, Math.log10(DAW_VR_FREQ_MIN) + (x / DAW_VR_GRAPH_W) * (Math.log10(DAW_VR_FREQ_MAX) - Math.log10(DAW_VR_FREQ_MIN))); }
        function dawVrDbToY(db) { return DAW_VR_GRAPH_H / 2 - (db / DAW_VR_DB_RANGE) * (DAW_VR_GRAPH_H / 2); }
        function dawVrYToDb(y) { return ((DAW_VR_GRAPH_H / 2 - y) / (DAW_VR_GRAPH_H / 2)) * DAW_VR_DB_RANGE; }
        ...
        // Faint decorative "input spectrum" trace behind the formant curve — live via t
        function dawVrNoisePath(key, t) {
            const rand = dawVrSeededRand('vr-noise-' + key);
            const bars = 70;
            let d = `M 0 ${(DAW_VR_GRAPH_H - 4).toFixed(1)}`;
            for (let i = 0; i <= bars; i++) {
                const x = (i / bars) * DAW_VR_GRAPH_W;
                const base = rand();
                const live = Math.sin(t / 500 + i * 0.6) * 0.15;
                const n = Math.max(0, Math.min(1, base * 0.7 + live + 0.15));
                const y = DAW_VR_GRAPH_H - 4 - n * (DAW_VR_GRAPH_H - 30);
        ...
        function dawVrGraphInner(key, t) {
            const freqTicks = [50, 100, 200, 500, '1k', '2k', '5k', '10k'];
            const freqVals = [50, 100, 200, 500, 1000, 2000, 5000, 10000];
            const gridLines = freqVals.map((f, i) => {
                const x = dawVrFreqToX(f);
                return `<line x1="${x.toFixed(1)}" y1="0" x2="${x.toFixed(1)}" y2="${DAW_VR_GRAPH_H}" stroke="rgba(47,208,255,0.07)" stroke-width="1"/>
                        <text x="${(x + 3).toFixed(1)}" y="${DAW_VR_GRAPH_H - 4}" fill="#2fd0ff" opacity="0.4" font-size="8" font-family="monospace">${freqTicks[i]}</text>`;
            }).join('');
            const zeroY = dawVrDbToY(0);

            const nodes = dawVrNodePositions(key);
            const sorted = [...nodes].sort((a, b) => a.x - b.x);
            const curvePts = [[0, zeroY], ...sorted.map(n => [n.x, n.y]), [DAW_VR_GRAPH_W, zeroY]];
            const curveStr = curvePts.map(p => `${p[0].toFixed(1)},${p[1].toFixed(1)}`).join(' ');

            const dots = nodes.map(n => `
                <circle class="dawvr-node" data-field="${n.field}" cx="${n.x.toFixed(1)}" cy="${n.y.toFixed(1)}" r="6" fill="${n.color}" stroke="#000" stroke-width="1.5" style="cursor:ns-resize;"/>`).join('');

            return `
                <rect x="0" y="0" width="${DAW_VR_GRAPH_W}" height="${DAW_VR_GRAPH_H}" fill="#050505"/>
                ${gridLines}
                <line x1="0" y1="${zeroY.toFixed(1)}" x2="${DAW_VR_GRAPH_W}" y2="${zeroY.toFixed(1)}" stroke="rgba(47,208,255,0.15)" stroke-width="1"/>
                <path d="${dawVrNoisePath(key, t)}" fill="none" stroke="rgba(255,255,255,0.18)" stroke-width="1"/>
                <polyline points="${curveStr}" fill="none" stroke="#4ade80" stroke-width="2" style="filter: drop-shadow(0 0 3px rgba(74,222,128,0.6));"/>
                ${dots}`;
        }

        window.dawVrNodeDrag = function(e, key) {
            e.preventDefault();
        ...
        window.dawVrRafIds = window.dawVrRafIds || {};
        window.dawVrAnimate = function(key) {
            const sk = eq8SafeKey(key);
            if (window.dawVrRafIds[sk]) return;
            const step = (t) => {
                const svg = document.getElementById(`dawvr-graph-${sk}`);
                if (!svg) { delete window.dawVrRafIds[sk]; return; }
                svg.innerHTML = dawVrGraphInner(key, t);
                window.dawVrRafIds[sk] = window.requestAnimationFrame(step);
            };
            window.dawVrRafIds[sk] = window.requestAnimationFrame(step);
        };

        const DAW_VR_KNOB_CFG = {
            fund: { min: -DAW_VR_DB_RANGE, max: DAW_VR_DB_RANGE, fmt: v => (v >= 0 ? '+' : '') + v.toFixed(0) + 'dB', color: DAW_VR_NODE_COLORS[0] },
            second: { min: -DAW_VR_DB_RANGE, max: DAW_VR_DB_RANGE, fmt: v => (v >= 0 ? '+' : '') + v.toFixed(0) + 'dB', color: DAW_VR_NODE_COLORS[1] },
            third: { min: -DAW_VR_DB_RANGE, max: DAW_VR_DB_RANGE, fmt: v => (v >= 0 ? '+' : '') + v.toFixed(0) + 'dB', color: DAW_VR_NODE_COLORS[2] },
            nthPos: { min: 200, max: 12000, fmt: v => v >= 1000 ? (v / 1000).toFixed(1) + 'kHz' : Math.round(v) + 'Hz', color: DAW_VR_NODE_COLORS[3] },
            gain: { min: -DAW_VR_DB_RANGE, max: DAW_VR_DB_RANGE, fmt: v => (v >= 0 ? '+' : '') + v.toFixed(0) + 'dB', color: '#9ca3af' },
            q: { min: 0, max: 100, fmt: v => Math.round(v) + '%', color: '#2dd4bf' },
        ...
        function dawVrKnob(key, sk, field, label) {
            ...
            return `
            <div class="flex flex-col items-center gap-1">
                <div id="dawvr-knob-${field}-${sk}" class="relative rounded-full bg-black cursor-ns-resize select-none flex-shrink-0"
                     style="width:42px; height:42px; border:2px solid ${c.color}88; box-shadow: inset 0 0 6px rgba(0,0,0,0.8), 0 0 6px ${c.color}33;"
        ...
        window.dawVrPanel = function(key, plugin) {
            const s = dawVrGetState(key);
            const sk = eq8SafeKey(key);
            try { window.dawVrAnimate(key); } catch (e) {}
            ...
            <div class="flex items-start justify-between gap-2 mb-1">
                ...
                <span class="relative inline-block flex-shrink-0">
                    <button onclick="event.stopPropagation(); window.dawVrTogglePresetMenu('${key}')" class="flex items-center gap-1 text-[8px] font-black uppercase tracking-widest neon-blue-text border border-[rgba(47,208,255,0.5)] rounded px-2 py-1 bg-black/50 hover:bg-[#2fd0ff]/15 transition-colors max-w-[150px]">
                        <span id="dawvr-badge-${sk}" class="truncate">${window.dawFxActivePresetLabel[key] || 'factory preset'}</span>
                        <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" class="flex-shrink-0"><path d="M6 9l6 6 6-6"/></svg>
                    </button>
                    <div id="dawvr-preset-menu-${sk}" class="hidden-section"></div>
                </span>
            </div>
            <div class="inline-block text-[8px] neon-blue-text uppercase font-black tracking-widest mt-1 border border-[rgba(47,208,255,0.35)] rounded px-1.5 py-0.5">${plugin.category}</div>

            <div class="mt-2 rounded-lg overflow-hidden" style="border:1px solid rgba(47,208,255,0.35); box-shadow: inset 0 0 12px rgba(47,208,255,0.08);">
                <svg id="dawvr-graph-${sk}" viewBox="0 0 ${DAW_VR_GRAPH_W} ${DAW_VR_GRAPH_H}" preserveAspectRatio="none" style="width:100%; height:190px; display:block;"
                     onmousedown="window.dawVrNodeDrag(event,'${key}')" ontouchstart="window.dawVrNodeDrag(event,'${key}')">${dawVrGraphInner(key, performance.now())}</svg>
            </div>

            <div class="flex items-center justify-center gap-3 mt-3">
                ${dawVrKnob(key, sk, 'fund', '1st (fund.)')}
                ${dawVrKnob(key, sk, 'second', '2nd')}
                ${dawVrKnob(key, sk, 'third', '3d')}
                ${dawVrKnob(key, sk, 'nthPos', 'Nth pos')}
                ${dawVrKnob(key, sk, 'gain', 'Gain')}
                ${dawVrKnob(key, sk, 'q', 'Q')}
                ${dawVrKnob(key, sk, 'outVol', 'Out vol')}
            </div>`;
        };

        window.dawVrTogglePresetMenu = function(key) {
            const sk = eq8SafeKey(key);
            const el = document.getElementById(`dawvr-preset-menu-${sk}`);
            if (!el) return;
            const isHidden = el.classList.contains('hidden-section');
            if (isHidden) { window.dawVrRenderPresetMenu(key); el.classList.remove('hidden-section'); }
            else el.classList.add('hidden-section');
        };

        window.dawVrRenderPresetMenu = function(key) {
            const sk = eq8SafeKey(key);
            const el = document.getElementById(`dawvr-preset-menu-${sk}`);
            if (!el) return;
            const userPresets = (window.dawFxUserPresets['vocal-rider'] || []);
            const activeLabel = window.dawFxActivePresetLabel[key];
            el.innerHTML = `
            <div class="absolute top-full right-0 mt-1.5 w-56 z-20 bg-black rounded-lg overflow-y-auto slick-scroll" style="border:1px solid #2fd0ff; box-shadow: 0 0 16px rgba(47,208,255,0.4), inset 0 0 2px rgba(47,208,255,0.45); max-height:220px;" onclick="event.stopPropagation()">
                <button onclick="window.dawVrApplyPreset('${key}', null)" class="w-full text-left px-3 py-2 text-[9px] font-black uppercase tracking-widest neon-blue-text hover:bg-[#2fd0ff]/15 transition-colors border-b border-[rgba(47,208,255,0.25)]">Reset to factory default</button>
                <button onclick="window.dawVrApplyPreset('${key}', null)" class="w-full flex items-center gap-2 text-left px-3 py-2 text-[9px] font-black uppercase tracking-widest transition-colors border-b border-[rgba(47,208,255,0.15)] ${!activeLabel ? 'bg-[#2fd0ff]/20 neon-blue-text' : 'text-gray-400 hover:bg-white/5'}">
                    <span class="w-3 flex-shrink-0">${!activeLabel ? '✓' : ''}</span> No preset
                </button>
                ${userPresets.length ? `<div class="px-3 py-1.5 text-[7px] text-gray-500 uppercase tracking-[0.2em] border-b border-[rgba(47,208,255,0.15)]">---- User Presets ----</div>` : ''}
                ${userPresets.map((p, i) => `
                <button onclick="window.dawVrApplyPreset('${key}', ${i})" class="w-full flex items-center gap-2 text-left px-3 py-2 text-[9px] font-bold tracking-wide transition-colors ${activeLabel === p.name ? 'bg-[#2fd0ff]/20 neon-blue-text' : 'text-gray-300 hover:bg-white/5 hover:text-[#2fd0ff]'}">
                    <span class="w-3 flex-shrink-0">${activeLabel === p.name ? '✓' : ''}</span> <span class="truncate">${p.name}</span>
                </button>`).join('')}
            </div>`;
        };

        window.dawVrApplyPreset = function(key, presetIndex) {
            if (presetIndex === null || presetIndex === undefined) {
                window.dawVrState[key] = dawVrDefaultState();
                delete window.dawFxActivePresetLabel[key];
            } else {
        ...
        window.dawSpPanel = function(key, plugin) {
            ...
            const tickY = v => meterTopPad + (v / 60) * (meterH - meterTopPad - meterBotPad);
            const gainTickRows = gainTicks.map(v => `
                <div class="flex items-center justify-between" style="position:absolute; left:0; right:0; top:${tickY(v).toFixed(1)}px; transform:translateY(-50%);">
            ...
            <div class="flex gap-2 mt-2">
                ...
                <div class="flex flex-col items-center flex-shrink-0" style="width:90px;">
                    <span class="text-[7px] neon-blue-text uppercase font-black tracking-widest mb-1">Gain</span>
                    <div class="relative rounded" style="width:100%; height:${meterH}px; border:1px solid rgba(47,208,255,0.3); background:#050505;">
                        ${gainTickRows}
        ...
        window.dawFxTopMenuSavePreset = function() {
            ...
            } else if (sel.plugin.id === 'stereo-imager') {
        ...
        // ============================================================
        // BACKEND CONFIG — set this once you've deployed server.js (e.g. to
        // Render). Leave as '' for local dev, where the frontend and backend
        // share an origin and relative paths work fine.
        // ============================================================
        window.SOVEREIGN_BACKEND_URL = 'https://sick-team-lab-backend.onrender.com';

        // Polls our own /api/sovereign/produce/:taskId (never AetherWave directly —
        // the X-AW-Key never leaves the server) every 5s until the task is
        // draft_ready or failed.
        function pollSovereignTask(taskId, { intervalMs = 5000, timeoutMs = 10 * 60 * 1000 } = {}) {
            const startedAt = Date.now();
            return new Promise((resolve, reject) => {
                const tick = () => {
                    fetch(window.SOVEREIGN_BACKEND_URL + '/api/sovereign/produce/' + encodeURIComponent(taskId))
                        .then(r => r.json())
                        .then(task => {
                            if (task.status === 'draft_ready') return resolve(task);
                            if (task.status === 'failed') return reject(new Error(task.error || 'Generation failed'));
                            if (Date.now() - startedAt > timeoutMs) return reject(new Error('Timed out waiting on AetherWave.'));
        ...
        window.generateTrack = function() {
            ...
            // Prompt mode gives a theme/description, not literal lyrics — AetherWave's
            // custom mode needs actual lyric text in `prompt`, so send the description
            // through as-is; it'll read as a loose vocal direction rather than fixed lines.
            if (window.lyricsMode === 'prompt') lyricsForSong = document.getElementById('custom-lyrics-prompt').value;
            // Instrumental mode has no lyrics box at all, but the backend still requires
            // non-empty `lyrics` text — the title stands in; AetherWave ignores it when
            // instrumental: true is set below.
            if (isInstrumental) lyricsForSong = title;

            btn.disabled = true;
            btn.innerText = 'FORGING...';
            btn.classList.add('opacity-60', 'cursor-not-allowed');

            const overrides = { title, instrumental: isInstrumental };
            // vocalGender toggle is optional — leave the Sovereign Template's locked
            // default ('f' / Lexi Con) in place unless the user explicitly picked one.
            if (window.vocalGender === 'male') overrides.vocalGender = 'm';
            if (window.vocalGender === 'female') overrides.vocalGender = 'f';

            fetch(window.SOVEREIGN_BACKEND_URL + '/api/sovereign/produce', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ lyrics: lyricsForSong, overrides })
            })
            .then(r => r.json())
            .then(result => {
                if (result.blocked) {
                    // Credit / rate-limit / missing-key guardrail — surface it to the HUD.
                    throw new Error(result.message || 'Production blocked.');
                }
                btn.innerText = 'PRODUCING...';
                return pollSovereignTask(result.taskId);
            })
            .then(task => {
                if (!task.draftTrackIds || !task.draftTrackIds.length) {
                    throw new Error('Generation completed but no drafts came back.');
                }
                // AetherWave always returns two takes — pull metadata for each and
                // drop both into Your Creations as drafts awaiting review.
                return Promise.all(task.draftTrackIds.map(id =>
                    fetch(window.SOVEREIGN_BACKEND_URL + '/api/tracks/' + id).then(r => r.json()).then(meta => ({ id, meta }))
                ));
            })
            .then(drafts => {
                btn.innerText = 'COMPLETE ✨';
                drafts.forEach(({ id, meta }) => {
                    window.addCreation(meta.title || title, lyricsForSong, window.SOVEREIGN_BACKEND_URL + '/api/tracks/' + id + '/audio');
                });
                setTimeout(() => {
                    btn.disabled = false;
        ...
        const DAW_AUTOMATION_LANE_H = 90; // px height of an expanded track's volume-automation lane
        ...
        window.dawAutomationExpanded = {}; // trackId -> bool, whether the volume automation lane is shown under that track
        ...
        window.dawStartMeterLoop = function() {
            ...
            const tick = () => {
                ...
                if (pCtx && pCtx.type === 'daw' && pCtx.selectedIndex !== null) {
                    ...
                    // Same pattern for an open Stereo Imager panel — drive its L/R
                    // output meters off the selected track's channel level.
                    if (pPlugin && pPlugin.id === 'stereo-imager') {
                        const isk = eq8SafeKey(`${pCtx.trackId}::${pCtx.selectedIndex}`);
                        const base = window.dawMeterPeaks[pCtx.trackId === 'master' ? 'master' : ('t-' + pCtx.trackId)] || 0;
                        const lVal = base;
                        const rVal = Math.max(0, Math.min(100, base * (0.88 + Math.random() * 0.18)));
                        window.dawUpdateLed('dawimg-led-L-' + isk, lVal);
        ...
        window.renderDawTracks = function() {
            ...
            const dawRowHeight = (t) => {
                const fxList = t.fx || [];
                const isFxExpanded = window.dawHeaderFxExpanded[t.id] && fxList.length;
                const isAutoExpanded = window.dawAutomationExpanded[t.id];
                let h = DAW_ROW_H;
                if (isFxExpanded) h += 138;
                if (isAutoExpanded) h += DAW_AUTOMATION_LANE_H;
                return h;
            };
            ...
            headers.innerHTML = window.dawTracks.map((t, i) => {
                ...
                const autoPts = dawAutomationPoints(t);
                const isAutoOn = !!window.dawAutomationExpanded[t.id];
                return `
                <div class="daw-track-header-row" oncontextmenu="window.openDawTrackContextMenu(event,'${t.id}')" style="height:${dawRowHeight(t)}px;">
                    <div class="flex items-center gap-2">
                        ...
                        <button onclick="window.toggleDawTrackAutomation('${t.id}')" id="daw-auto-${t.id}" class="daw-chip-btn ${isAutoOn ? 'fx-assigned' : ''}" title="${autoPts.length ? autoPts.length + ' volume automation point(s)' : 'Volume automation'}">A${autoPts.length ? ' ' + autoPts.length : ''}</button>
            ...
            lanes.innerHTML = window.dawTracks.map((t, i) => {
                const isAutoExpanded = window.dawAutomationExpanded[t.id];
                return `
                <div class="relative border-b border-white/5 flex flex-col overflow-hidden" oncontextmenu="window.openDawTrackContextMenu(event,'${t.id}')" style="height:${dawRowHeight(t)}px; overflow:hidden; z-index:1;">
                    <div id="clip-wrap-${t.id}" class="relative w-full flex-shrink-0" style="height:calc(100% - ${isAutoExpanded ? DAW_AUTOMATION_LANE_H : 0}px); transform:translateX(${window.dawClipOffsets[t.id] || 0}px); overflow:hidden; z-index:1;">
                    ...
                    ${isAutoExpanded ? `
                    <div id="daw-automation-wrap-${t.id}" class="relative w-full flex-shrink-0 border-t border-[rgba(47,208,255,0.15)]" style="height:${DAW_AUTOMATION_LANE_H}px; background:rgba(0,0,0,0.35);">
                        ${dawAutomationLaneSvg(t, DAW_AUTOMATION_LANE_H)}
                    </div>` : ''}
                </div>`;
            }).join('');
        ...
        // ============================================================
        // VOLUME AUTOMATION — a per-track envelope lane (toggle with the "A"
        // header chip) whose points drive the track's mixer fader live during
        // playback/scrubbing, the same way a REAPER/Ableton volume envelope
        // moves its channel fader. Points live in track.automation.volume as
        // [{ t: seconds, v: 0-100 }], sorted by time. The lane is drawn with a
        // viewBox of 0..1000 (x) by 0..100 (y) and preserveAspectRatio="none",
        // so it always stretches to the lane's actual rendered width/height
        // regardless of zoom — no pixel math needs to match the ruler exactly,
        // only the same total-seconds domain the ruler already uses.
        // ============================================================
        function dawAutomationPoints(t) {
            if (!t.automation) t.automation = {};
            if (!Array.isArray(t.automation.volume)) t.automation.volume = [];
            return t.automation.volume;
        }

        function dawAutomationTotalSeconds() {
            const { totalBars } = dawComputeTimelineLayout();
            const bpm = parseFloat(window.dawBpm) || 120;
            const secPerBar = (60 / bpm) * 4;
            return Math.max(1, totalBars * secPerBar);
        }

        const DAW_AUTO_Y_PAD = 8; // percent of the 0-100 viewBox reserved top/bottom so nodes at 0%/100% aren't clipped
        function dawAutoValToY(v) {
            const clamped = Math.max(0, Math.min(100, Number(v) || 0));
            return DAW_AUTO_Y_PAD + (100 - clamped) * (100 - 2 * DAW_AUTO_Y_PAD) / 100;
        }
        function dawAutoYToVal(yPct) {
            const usable = 100 - 2 * DAW_AUTO_Y_PAD;
            const v = 100 - ((yPct - DAW_AUTO_Y_PAD) / usable) * 100;
        ...
        window.dawAutomationValueAt = function(track, time) {
            ...
            if (time >= pts[pts.length - 1].t) return pts[pts.length - 1].v;
        ...
        // Re-draws just the <svg> contents for one track's automation lane — used for
        // drag feedback and live playback so we never have to rebuild the whole
        // arrangement (which would also blow away the WaveSurfer instances).
        window.renderDawAutomationLane = function(trackId) {
            const track = window.dawTracks.find(x => x.id === trackId);
            const wrap = document.getElementById('daw-automation-wrap-' + trackId);
            if (!track || !wrap) return;
            wrap.innerHTML = dawAutomationLaneSvg(track, DAW_AUTOMATION_LANE_H);
        };

        // Called after zoom changes (the total-bars/seconds domain the lane's x-axis
        // is scaled against can change), so every visible lane stays aligned with the ruler.
        window.renderDawAllAutomationLanes = function() {
            window.dawTracks.forEach(t => { if (window.dawAutomationExpanded[t.id]) window.renderDawAutomationLane(t.id); });
        };

        window.toggleDawTrackAutomation = function(trackId) {
            window.dawAutomationExpanded[trackId] = !window.dawAutomationExpanded[trackId];
        ...
        // automation curve at the current transport time — called every playback
        // frame and on every seek/scrub, so the fader visibly moves on its own,
        ...
        window.dawApplyZoom = function() {
            ...
            window.renderDawAllAutomationLanes(); // the lane x-axis is scaled against the same total-bars domain as the ruler
        };
                ...
                <div class="daw-mixer-strip" style="width:84px;${isSelected ? ' box-shadow: inset 0 0 0 1.5px rgba(47,208,255,0.7); background: rgba(47,208,255,0.04);' : ''}" onclick="window.selectDawTrack('${t.id}')" ondragover="window.dawAllowDrop(event)" ondrop="window.dawDropOnStrip(event,'${t.id}')">
                    ...
                    <div class="flex items-end gap-1">
                        ...
                        <div class="daw-fader-track">
                            <input type="range" id="daw-fader-${t.id}" min="0" max="100" value="${t.volume}" class="daw-fader-input" oninput="setDawVolume('${t.id}', this.value)">
                        </div>
        ...
        window.setDawVolume = function(trackId, value) {
            ...
            // Keep the on-screen fader in sync when volume is set programmatically
            // (automation playback/scrubbing) rather than by the user dragging it.
            const fader = document.getElementById('daw-fader-' + trackId);
            if (fader && Number(fader.value) !== Number(value)) fader.value = value;
        };
        ...
        window.dawSaveProject = async function(evt) {
            ...
            try {
                const tracks = window.dawTracks.map(t => ({
                    ...
                    volume: t.volume, fx: t.fx || [], automation: t.automation || null, file: window.dawFiles[t.id] || null
                }));
                const project = {
                    tracks,
                    bpm: window.dawBpm,
        ...
        window.dawLoadProject = async function(evt) {
            ...
            try {
                ...
                window.dawTracks = project.tracks.map(t => ({ id: t.id, name: t.name, color: t.color, muted: t.muted, solo: t.solo, volume: t.volume, fx: t.fx || [], automation: t.automation || null }));
                window.dawAutomationExpanded = {};
                window.dawFiles = {};
                window.dawUrls = {};
        ...
        // This is only a fallback for when the ElevenLabs neural voice (Lexi-Con's real voice)
        ...
        // --- ElevenLabs neural TTS (via our own backend proxy): smoother, non-robotic
        // voice for Lexi-Con. Routes through SOVEREIGN_BACKEND_URL/api/vocal/render so the
        // ElevenLabs key stays server-side — the frontend never touches it directly.
        // Falls back to the browser's built-in speech synthesis, screened to a known female
        // voice, if the backend request fails for any reason.
        let relaySpeechGeneration = 0; // bumped on every new reply so a stale queued chunk can never talk over a newer one

        // The backend allows up to 5000 chars per request, but chunking on sentence
        // boundaries still gives more natural breathing gaps between sentences, so we keep it
        // — just with a much higher ceiling than Orpheus's old 200-char cap.
        const ELEVENLABS_TTS_CHAR_LIMIT = 1000;
        const ELEVENLABS_TTS_VOICE_KEY = 'bella'; // free-tier-compatible default account voice — see vocal-render.js notes
        const ELEVENLABS_TTS_INTER_CHUNK_PAUSE_MS = GROQ_TTS_INTER_CHUNK_PAUSE_MS;

        function sfcChunkForElevenLabs(text) {
            ...
            for (let sentence of sentences) {
                ...
                while (sentence.length > ELEVENLABS_TTS_CHAR_LIMIT) {
                    if (current) { chunks.push(current.trim()); current = ''; }
                    let cut = sentence.slice(0, ELEVENLABS_TTS_CHAR_LIMIT);
                    const lastSpace = cut.lastIndexOf(' ');
                    if (lastSpace > 0) cut = cut.slice(0, lastSpace);
                    chunks.push(cut.trim());
                    sentence = sentence.slice(cut.length).trim();
                }
                if ((current + ' ' + sentence).trim().length > ELEVENLABS_TTS_CHAR_LIMIT) {
        ...
        async function sfcFetchElevenLabsAudio(text) {
            const response = await fetch(window.SOVEREIGN_BACKEND_URL + '/api/vocal/render', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ voiceKey: ELEVENLABS_TTS_VOICE_KEY, text })
            });
            if (!response.ok) {
        ...
        function sfcPrefetchElevenLabsChunk(text) {
            return sfcFetchElevenLabsAudio(text)
        ...
        async function sfcPlayElevenLabsSpeech(text, generation) {
            const chunks = sfcChunkForElevenLabs(text);
            if (!chunks.length) return false;

            let nextChunkPromise = sfcPrefetchElevenLabsChunk(chunks[0]);
            ...
            for (let i = 0; i < chunks.length; i++) {
                ...
                if (!result.ok) {
                    console.error('ElevenLabs TTS error:', result.err);
                ...
                if (i + 1 < chunks.length) {
                    nextChunkPromise = sfcPrefetchElevenLabsChunk(chunks[i + 1]);
                }
                ...
                if (i + 1 < chunks.length && generation === relaySpeechGeneration && !window.relayMuted) {
                    await new Promise(resolve => setTimeout(resolve, ELEVENLABS_TTS_INTER_CHUNK_PAUSE_MS));
                }
            }
            return true;
        }
        ...
        window.speakRelayMessage = async function(text) {
            ...
            let spoke = false;
            try {
                spoke = await sfcPlayElevenLabsSpeech(clean, myGeneration);
            } catch (err) {
        ...
        window.addSignal = function(persona, message, mediaUrl, mediaType) {
            ...
            // Final safety net: strip ANY leading "PERSONA:" label (any known persona, not just
            // this one, in case the model echoes a different/mismatched one), looped in case it's
            // doubled — regardless of what parsing upstream already tried. The UI renders the
            // persona label itself (see renderRelayFeed), so the message text should never also
            // start with one.
            if (typeof message === 'string') {
                const personaNames = Object.keys(RELAY_PERSONA_COLORS)
                    .sort((a, b) => b.length - a.length) // longest first, so "CODEX KEEPER" beats a partial "CODEX" match
                    .map(name => name.replace(/[-/\\^$*+?.()|[\]{}]/g, '\\$&'))
                    .join('|');
                const anyLabelPattern = new RegExp('^[\\s*_]*(?:' + personaNames + ')[\\s*_]*:\\s*', 'i');
                let stripped = message;
                for (let i = 0; i < 5; i++) {
                    const next = stripped.replace(anyLabelPattern, '').trim();
