        ...
        // === SOVEREIGN PERSONA ENGINE (ISOLATED) ===
window.switchSyndicatePersona = function(id) {
    const team = {
        "01": { n: "ORACLE", c: "text-yellow-400" },
        "02": { n: "OPERATOR", c: "text-purple-400" },
        "03": { n: "LEXI-CON", c: "lexi-identity-text font-bold" },
        "04": { n: "BOND", c: "text-white opacity-70" },
        "05": { n: "STASIS", c: "text-gray-400" },
        "06": { n: "SPARK", c: "text-purple-300" },
        "07": { n: "CODEX KEEPER", c: "text-white" },
        "08": { n: "K-VOLT", c: "text-blue-400" }
    };
    const agent = team[id];
    if (!agent) return;

    const hub = document.querySelector('.message-content');
    if (hub) {
        hub.innerHTML = `<div class="terminal-entry" style="animation:fadeIn 0.4s ease;">
            <span class="${agent.c}">[ ${agent.n} ]:</span> 
            <span style="color:rgba(255,255,255,0.8); margin-left:10px;">Channel Secured. Architect, standing by...</span>
        </div>`;
    }
};// === SOVEREIGN GHOST CLICKER ===
// This "listens" for clicks on your 01-08 vertical list without lagging the browser.
document.addEventListener('click', function(e) {
    const target = e.target;
    // Get the text inside the element you clicked
    const text = target.innerText ? target.innerText.trim() : "";
    
    // Pattern match: Does it start with 01 //, 02 //, etc?
    const match = text.match(/^(0[1-8]) \/\//);
    
    if (match) {
        console.log("[SYSTEM]: Node Triggered -> " + match[1]);
        // Call the persona switcher we just built
        window.switchSyndicatePersona(match[1]);
        
        // Add a temporary glow effect to the clicked item
        target.style.textShadow = "0 0 15px #00f2ff";
        setTimeout(() => { target.style.textShadow = "none"; }, 500);
    }
});
