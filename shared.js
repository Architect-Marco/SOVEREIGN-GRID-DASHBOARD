            ...
            headers.innerHTML = window.dawTracks.map((t, i) => {
                ...
                <div class="daw-track-header-row" oncontextmenu="window.openDawTrackContextMenu(event,'${t.id}')" style="height:${dawRowHeight(t)}px;">
                    ...
                    <div class="flex items-center gap-2 pl-6 min-w-0">
                        <input type="file" id="daw-upload-${t.id}" accept="audio/*,.mp3,.wav,.ogg,.oga,.m4a,.aac,.flac,.aiff,.wma,.webm" style="position:absolute; width:1px; height:1px; padding:0; margin:-1px; overflow:hidden; clip:rect(0,0,0,0); white-space:nowrap; border:0;" onchange="handleDawUpload(event, '${t.id}')">
                        <label for="daw-upload-${t.id}" class="text-gray-500 hover:text-[#2fd0ff] transition-colors flex-shrink-0 cursor-pointer" style="position:relative; z-index:5; padding:4px; margin:-4px;" title="Upload">${DAW_UPLOAD_ICON}</label>
